                        ...
                        <div class="row row-narrow row-20">
                            ...
                            <div class="col-md-6" v-if="incription_concepts.length > 0">
                                <div class="form-wrap2">
                                    ...
                                    <select name="" id="" v-model="form.incription_concept" disabled style="width: 100%; min-height: 78px; padding: 6%;" required>
                            ...
                            <div class="col-md-6" v-if="certificate_concepts.length > 0 && is_certificate == true">
                                <div class="form-wrap2">
                                    ...
                                    <select name="" id="" v-model="form.certificate_concept" disabled style="width: 100%; min-height: 78px; padding: 6%;" required>
                                        <option value="" disabled>-- SELECCIONAR UNA OPCIÓN --</option>
...
export default {
    ...
    methods: {
                ...
                setTimeout(async () => {
                    ...
                    if (!resp.data.error) {
                        ...
                        if (Object.keys(resp.data.data).length > 0){
                            ...
                            this.arrayRoles = [];
                            this.assignInscriptionConcept(resp.data.data.roles);
                            if (this.is_certificate) {
                                this.assignCertificateConcept(resp.data.data.roles);
                            }
                            this.arrayRolesOrigin = resp.data.data.roles;
        ...
        assignInscriptionConcept(rols){
            ...
            this.form.incription_concept = id_concepto_insc;
        },

        assignCertificateConcept(rols){
            let rol_descripcion = '';
            // si no tiene roles, es externo
            if (rols.length == 0) {
                rol_descripcion = 'Publico General'
        ...
        getDescriptionByPriority(data) {
            const priorities = [
                { priority: "Estudiante EGRESADO", returnVal: "Egresado UCT" },
                { priority: "Docente", returnVal: "Docente UCT" },
                { priority: "Administrativo", returnVal: "Administrativo UCT" },
                { priority: "Estudiante NOEGRESADO", returnVal: "Publico General" }
            ];

            for (let priorityObj of priorities) {
                const result = data.find(object => object.description.includes(priorityObj.priority));
                if (result) {
                    return priorityObj.returnVal;
                }
            }
        ...
        formatIdByDescription(objects, description) {
            const foundObject = objects.find(obj => obj.descripcion === description);
            if (foundObject) {
                return "d" + foundObject.id;
            } else {
                return null;
            }
        },
    ...
    watch:{
        is_certificate(val){
            ...
            if (val == true) {
                this.assignCertificateConcept(this.arrayRolesOrigin)
            }
        }
    }
}
